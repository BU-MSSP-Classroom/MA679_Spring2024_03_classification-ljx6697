...
Your code:
```{r,echo=TRUE}
glm_fit <- glm(SURVIVED_bin~AGE_TRUNK+SEX,family=binomial,data= Train_COVID)

logit_P = predict(glm_fit,Test_COVID,type="response")
...
```{r,echo=TRUE}
roc_score=roc(Test_COVID$SURVIVED_bin, logit_P)
plot(roc_score ,main ="ROC curve -- Logistic Regression ")
```
...
(1)
Accuracy: 0.924129737889355
Precision: 0.930201734347746
Sensitivity: 0.992606314551402
Specificity: 0.0588541279631419
...
Your code:
```{r,echo=TRUE}
M <-cor(Covid_Data[,c("FEMALE", "AGE_TRUNK", "SURVIVED_bin")],use="pairwise.complete.obs")
```
...
Based on MY model, the factor "SEX" seems to be important in deciding if a patient survives or not.
...
plot(Auto$mpg01, Auto$cylinders, xlab = "mpg01", ylab = "Cylinders", main = "Cylinders vs. mpg01", pch = 19)
plot(Auto$mpg01, Auto$displacement, xlab = "mpg01", ylab = "Displacement", main = "Displacement vs. mpg01", pch = 19)
plot(Auto$mpg01, Auto$horsepower, xlab = "mpg01", ylab = "Horsepower", main = "Horsepower vs. mpg01", pch = 19)
plot(Auto$mpg01, Auto$weight, xlab = "mpg01", ylab = "Weight", main = "Weight vs. mpg01", pch = 19)
plot(Auto$mpg01, Auto$acceleration, xlab = "mpg01", ylab = "Acceleration", main = "Acceleration vs. mpg01", pch = 19)

boxplot(mpg01 ~ origin, data = Auto, xlab = "Origin", ylab = "mpg01", main = "Boxplot of mpg01 by Origin")

par(mfrow = c(1, 1))


M <-cor(Auto[,c("mpg01", "cylinders","displacement","horsepower", "weight","acceleration","year","origin")],use="pairwise.complete.obs")
...
Based on the correlation matrix provided (M), we can observe the correlations between mpg01 and other features. The absolute values of the correlation coefficients indicate the strength and direction of the linear relationship between variables.
Here are the absolute correlation coefficients between mpg01 and the other features:
Cylinders: 0.7591939
...
Year: 0.4299042
Origin: 0.5136984
Features with higher absolute correlation coefficients with mpg01 are more likely to be useful in predicting mpg01. From the correlation matrix, we see that cylinders, displacement, weight have relatively higher absolute correlation coefficients with mpg01. These features seem to have stronger associations with the binary variable mpg01 and are therefore more likely to be useful in predicting it.
Therefore, the features most likely to be useful in predicting mpg01 based on the correlation coefficients are:
(1) Cylinders
(2) Displacement
(3) Weight
...
train_set<- Auto[samp_idx,]
test_set <- Auto[-samp_idx,]
...
```{r,echo=TRUE}
library(MASS)
lda_model <- lda(mpg01 ~ cylinders + displacement + weight, data = train_set)
lda_pred <- predict(lda_model, newdata = test_set)
predicted_classes <- lda_pred$class
test_error1 <- mean(predicted_classes != test_set$mpg01)
print(paste("Test error for LDA model:", test_error1))
```

Test error for LDA model: 0.0981818181818182
...
```{r,echo=TRUE}
library(MASS)
qda_model <- qda(mpg01 ~ cylinders + displacement + weight, data = train_set)
predicted_mpg01 <- predict(qda_model, newdata = test_set)
test_error2 <- mean(predicted_mpg01$class != test_set$mpg01)
print(paste("Test error for QDA model:", test_error2))
...
Test error for QDA model: 0.0981818181818182
~~~
...
Your code:
```{r,echo=TRUE}
glm_fit1<- glm(mpg01 ~ cylinders + displacement + weight, family=binomial,data= train_set)
summary(glm_fit1)
glm_fit1$converged

glm_fit2<- glm(mpg01 ~ weight, family=binomial,data= train_set)
summary(glm_fit2)
glm_fit2$converged


glm_fit1_pred <- predict(glm_fit1, newdata = test_set, type = "response")
glm_fit2_pred <- predict(glm_fit2, newdata = test_set, type = "response")

glm_fit1_pred_binary <- ifelse(glm_fit1_pred > 0.5, 1, 0)
glm_fit2_pred_binary <- ifelse(glm_fit2_pred > 0.5, 1, 0)

test_error_glm_fit1 <- mean(glm_fit1_pred_binary != test_set$mpg01)
test_error_glm_fit2 <- mean(glm_fit2_pred_binary != test_set$mpg01)

print(paste("Test error for glm_fit1:", test_error_glm_fit1))
print(paste("Test error for glm_fit2:", test_error_glm_fit2))
...
Test error for glm_fit1: 0.101818181818182

Test error for glm_fit2: 0.105454545454545
~~~
...
```{r,echo=TRUE}
library(e1071)
nb_model <- naiveBayes(mpg01 ~ cylinders + displacement + weight, data = train_set)

predictions <- predict(nb_model, test_set)

conf_matrix <- table(predictions, test_set$mpg01)
...
for (k in k_values) {
  ...
  knn_pred <- knn(train = train_set[, predictors], 
                  test = test_set[, predictors], 
                  cl = train_set$mpg01, 
                  k = k)
  error <- mean(knn_pred != test_set$mpg01)

  test_errors <- c(test_errors, error)
}

for (i in seq_along(k_values)) {
  cat("K =", k_values[i], "Test Error =", test_errors[i], "\n")
}
...
K = 1 Test Error = 0.1454545 
K = 3 Test Error = 0.1018182 
K = 5 Test Error = 0.1018182 
K = 7 Test Error = 0.1090909 
K = 9 Test Error = 0.1054545 

The value of K = 3 or 5  seems to perform the best on this data set.
